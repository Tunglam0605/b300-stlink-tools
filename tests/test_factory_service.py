...
def partial_target_output(protected: bool) -> CommandResult:
    state = "protected" if protected else "not protected"
    return result(
        "Info : Target voltage: 3.10\n"
        "Info : device id = 0x101f6413\n"
...
class FactoryServiceTests(unittest.TestCase):
    ...
    def test_partial_initial_wrp_report_blocks_before_erase_or_program(self) -> None:
        runner = ScriptedRunner([
            partial_target_output(False),
            result("** Verified OK **"),
            result("protection on"),
            target_output(True),
            result("reset complete"),
    ...
    def test_partial_report_after_wrp_off_restores_wrp_without_erase(self) -> None:
        runner = ScriptedRunner([
            target_output(True),
            result("protection off"),
            partial_target_output(False),
        ...
        self.assertNotIn("flash erase_sector", " ".join(rendered))
        self.assertNotIn("program {", " ".join(rendered))
