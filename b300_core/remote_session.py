...
class LocalCredentialStore:
    """Store remembered SSH passwords locally, encrypted at rest with a user-local key.

    This intentionally keeps credentials on the workstation only. It avoids plaintext
    configuration/logging while remaining dependency-light and cross-platform. It is
    not intended to defend against an attacker who already controls the same OS account.
    """
    ...
    def _read_all(self) -> Dict[str, str]:
        ...
        if not isinstance(payload, dict) or any(not isinstance(k, str) or not isinstance(v, str)
...
class _ForwardServer:
    ...
    def _serve(self) -> None:
        while not self._stop.is_set():
            ...
            threading.Thread(
                target=self._bridge, args=(client, channel),
                name="b300-ssh-%s-client" % self.name, daemon=True,
            ).start()

    def _bridge(self, client: socket.socket, channel) -> None:
...
class RemoteSession:
    """One authenticated SSH connection reused by GDB, TCL and other local forwards."""

    def __init__(self, profile: RemoteGatewayProfile, *,
                 credential_store: Optional[CredentialStore] = None,
                 ssh_client_factory: Optional[SshClientFactory] = None) -> None:
        ...
        self._forwards: Dict[str, _ForwardServer] = {}
        self._lock = threading.RLock()
    ...
    def state(self) -> RemoteSessionState:
        with self._lock:
            names = tuple(sorted(self._forwards))
            return RemoteSessionState(
                "connected" if self.connected else "disconnected",
                "%s@%s:%d" % (self.profile.user, self.profile.host, self.profile.port),
                self.connected,
                names,
            )
        ...
        with self._lock:
            ...
            try:
                ...
                if "authentication" in name or "auth" in name:
                    raise RemoteAuthenticationError("SSH authentication failed.") from error
                raise RemoteSessionError("SSH connection failed: %s" % error) from error
            self._client = client
    ...
    def open_forward(self, name: str, *, remote_port: int, local_port: int = 0,
        ...
        if not selected or any(ch not in "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_-" for ch in selected):
        ...
        with self._lock:
            ...
            if selected in self._forwards:
                return self._forwards[selected].forward
            try:
                ...
                raise RemoteForwardError("Unable to open local forwarding listener: %s" % error) from error
