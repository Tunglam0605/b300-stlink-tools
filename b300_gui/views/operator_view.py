...
class OperatorView(QWidget):
        ...
        if not probes:
            ...
            if self._target_info:
                self.probe_status_sub.setText(
                    f"Target: STM32F407 · Flash {self._target_info.flash_size_kb} KiB · "
                    f"Điện áp: {self._target_info.target_voltage:.2f}V · {self._target_info.protection_state}"
                )
            else:
    ...
    def set_target_info(self, info: TargetInfo) -> None:
        ...
        if self._probes:
            self.probe_status_sub.setText(
                f"Target: STM32F407 (0x{info.device_id:08X}) · Flash {info.flash_size_kb} KiB · "
                f"Điện áp: {info.target_voltage:.2f}V · {info.protection_state}"
            )
            self.stepper.set_step_state(0, "success", f"Target 0x{info.device_id:08X} OK")
        self._update_action_state()
    ...
    def _update_action_state(self) -> None:
        ...
        can_run = has_file and has_probe
