...
the CLI, or RDP changes. Normal Application flash never modifies WRP. The sole
exception is the separately authorized `provision-bootloader` factory workflow,
which is constrained to WRP and sectors 0--2 as documented in `SKILL.md`. Each
WRP change must be followed by reset/halt Option-Byte reload and read-back before
the next destructive step. RDP/security is read-only and must never be changed.
